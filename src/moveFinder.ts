...
import type { EntryStrategy, ExitStrategy } from './types/Strategy'
	...
	for (let i = 0; i < candles.length; i++) {
		...
		if (!combo) {
			...
			if (entryStrategy === 'EP') {
				...
				const new_start_sma = sma(
					[...candles].splice(i - (entryStrategy + 2), entryStrategy)
				)
